...
| `stock` | string | — | `1` / `true` / `in_stock` / `in-stock` → in-stock only. `0` / `false` / `out_of_stock` / `out-of-stock` → out-of-stock only. Empty = no filter. Uses the **same physical-qty logic as `available_stock`**: in-stock ⇔ qty > 0, out-of-stock ⇔ qty ≤ 0. With `Magento_InventoryApi` (MSI) enabled this is `SUM(inventory_source_item.quantity)` where `status=1`, per SKU; otherwise the legacy `cataloginventory_stock_item.qty` (stock_id=1). A SKU with no MSI rows counts as out-of-stock unless *MSI Legacy Fallback* is enabled in Admin → Combipower → TESS AI → Stock (then it falls back to the legacy qty). Reservations are NOT subtracted. |
...
| `items[].price` | float | no | New base price (excl. VAT), must be ≥ 0 when present. Omit to leave the base price unchanged |
